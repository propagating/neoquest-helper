// ==UserScript==
// @name        Neopets - Neoquest I Helper (firefox)
...
// NOTE: This is the firefox version. 
// It is not thoroughly tested
// Keybindings for attack, spirit of growth, and flee do not work.
...
if(neoquest.search('to start a new game in') == -1){ 
	...
	$(document).keypress(function(e){
		switch(e.which){	
			...
			// "n", do nothing / normal
			case 110:	if(GM_getValue("battle") == 1){ if (neoquest.search('Do nothing') != -1){$("a[href='javascript:;']").eq(0).click();}} else{walkingMode(1);}
			break;
			...
			// "l", action
			case 108: if(GM_getValue("battle") != 1) {//== 1){location.href = "javascript:setData('attack', 0);";}
			...
			// ";", absorb
			case 59:	if(neoquest.search('Cast') != -1){$("a[href='javascript:;']").eq(1).click();}
			break;
...
function runFighter(neoquest){
	if(GM_getValue("battle") == 0){     // not in a battle
		...
		if (neoquest.search('see what you found') != -1){
			if(debug){GM_log("END FIGHT");}
			location.href = "http://www.neopets.com/games/neoquest/neoquest.phtml";
		}
		else if(neoquest.search('Absorption') != -1){
			s=1;
			log = "CASTING ABSORPTION";
		}
		else if(neoquest.search('Spirit') != -1){ 
			if (Number(GM_getValue("health")) >= Number(GM_getValue("maxHealth")-60)){ // 'Spirit of Growth' heals 100, so make it 60 to be safe
				s=1;
			}
			else { log = "CASTING SPIRIT OF GROWTH"; }
		}  
		else if(neoquest.search('Spirit') == -1 && Number(GM_getValue("health")) < Number(GM_getValue("maxHealth")*potionAt)){ // if there's no SoG, wait till half life (or whatever user decided) to use a potion
			if(debug){GM_log("USING POTION");}
			pickPotion();
		} 
		// do whatever action is first:
		if(debug){GM_log(log);}
		setTimeout(function(){ $("a[href='javascript:;']").eq(s).click(); },Math.floor(Math.random()*1000+1000)); 
	}		
}
